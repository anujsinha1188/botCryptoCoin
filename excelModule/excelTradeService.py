...
class ExcelTradeService:
    ...
    checkOrderCount = FORCE_CHECK_AFTER_LOOP * 0
    ...
    def is_open_order_executed(self):
        try:
            ...
            elif self.openTradeSheet[LAST_ORDER_STATUS_ADDRESS].value != "filled":

                response_order_status = self.coinService.check_order_status(trade_id)
                trade_status = response_order_status.get("status")

                if trade_status != self.openTradeSheet[LAST_ORDER_STATUS_ADDRESS].value:
                    order_executed = True
                    self.openTradeSheet[LAST_ORDER_STATUS_ADDRESS].value = trade_status
        ...
        for coin in COIN_LIST:
            ...
            try:
                ...
                if self.openTradeSheet[COIN_NAME_ADDRESS].value == self.coin_tradeCode \
                    ...
                    if self.forceRun or count != self.openTradeSheet[OPEN_TRADE_COUNT_ADDRESS].value:
                        ...
                        self.process_coin()
                        if self.forceRun:
                            print("Sleep for 1 seconds in Force run.")
                            time.sleep(1)
                    else:
                        print("No force run or change in trade count: " + self.coin_tradeCode +
                              ". Trade count: " + str(count) + ".")
                else:
                    print("Sheet " + self.coin_tradeCode + " is not ready.")
            except Exception as error:
                self.excelErrorService.handel_error(error)
    ...
    def order_coin(self, side):
        ...
        try:
            ...
            self.openTradeSheet[OPEN_TRADE_COUNT_ADDRESS].value += 1
            self.openTradeWorkbook.save(OPEN_TRADE_FILE)
        ...
        try:
            ...
            if response.get('status') == "error":
                ...
                if response.get('code') == 400:
                    self.updateExcelService.update_error_status(response, self.openTradeSheet, self.openTradeRow)
                    self.openTradeWorkbook.save(OPEN_TRADE_FILE)
